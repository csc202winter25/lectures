...
def _find(node, key):
    ...
    # Else, do:
    #     Return the result of recursing on the given node's right.
    pass
...
def _remove(bst, node, key):
    # If the given node is None, then:
    #     (the given key does not exist within the given BST -- do whatever
    #      ought to be done with non-existent keys, e.g., raise an error)
    # Else if the given key is equal to the given node's key, then:
    ...
    #               ...thus, that key can *replace* the given node's key.
    #
